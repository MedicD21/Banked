...
/** Syncs balances + transactions for a single Plaid item. */
export async function syncItem(item: PlaidItemRow): Promise<SyncResult> {
  const sql = getSql();
...
export async function runDailySync(): Promise<SyncResult[]> {
  ...
  for (const item of items) {
    results.push(await syncItem(item as unknown as PlaidItemRow));
  }
